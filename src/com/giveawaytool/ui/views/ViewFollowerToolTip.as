package com.giveawaytool.ui.views {
	 ...
	 * @author LachhhSSD
	 */
	public class ViewFollowerToolTip extends ViewBase {
		public var logicOpen : LogicOnOffNextFrame;
		public var metaFollower:MetaFollower = MetaFollower.NULL;
		public var viewFollower : ViewFollowBtn;

		public function ViewFollowerToolTip(pScreen : UIBase, pVisual : DisplayObject) {
		...
		private function onClickAlert() : void {
			if(!UI_Menu.instance.logicNotification.logicVIPAccess.canSendFollowIfNotLive()) return ;
			UI_Menu.instance.logicNotification.logicSendToWidget.sendFollowAlert(MetaFollowAlert.create(metaFollower));
		}
		...
		public function registerFollowerView(v:ViewFollowerBtn):void  {
			screen.registerEventWithCallback(v.visual, MouseEvent.MOUSE_UP, new Callback(onClickFollowerView, this, [v]));
		}
		
		public function onClickFollowerView(view:ViewFollowBtn):void {
			if(view == null) return; 
			if(view.getMetaFollower().isNull()) return ;
			if(viewFollower) {
				viewFollower.visualBtn.deselect();
			}
			viewFollower = view;
			view.visualBtn.select();
			openWithFollower(view.getMetaFollower());
		...
		private function openWithFollower(m:MetaFollower):void {
			metaFollower = m;
			refresh();
		...
		private function onClickMarkAsNew() : void {
			metaFollower.isNew = !metaFollower.isNew;
			viewFollower.refresh();
			refresh();
		}

		override public function refresh() : void {
			super.refresh();
			if(metaFollower.isNull()) return ;
			
			screen.setNameOfDynamicBtn(btn2, metaFollower.isNew ? "Mark as read" : "Mark as new");
			
			btn1.deselect();
			btn2.deselect();
			btn4.deselect();
			msgTxt.text = metaFollower.getDescForToolTip();	
		}
